...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (indexPath.section == 0) { // section 0
    ...
    if (indexPath.row == 4) {
      ...
      btn.titleLabel.font = UIFontMake(18);
...
- (void)comparePassword:(QMUITableViewCell *)cell {
  ...
  [[[[RACSignal
    ...
    if ([self.users.password isEqualToString:self.users.passwordTwo]) {
      if ([self.users.password isEqualToString:@""])
        return @(1);
      else
        return @(2);
    }
    ...
    if (x.intValue == 2) {
      ...
      cell.textLabel.text = @"密码强度中";
      cell.textLabel.textColor = UIColorMakeWithHex(@"#1CD300");
    } else if (x.intValue == 0) {
      QMUILogInfo(@"same", @"result:false");
      cell.textLabel.text = @"密码不一致";
      cell.textLabel.textColor = UIColorMakeWithHex(@"#FF2D2D");
