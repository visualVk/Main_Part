...
- (void)removeFromMapView {
  ...
  if ([self.routePolylines count] > 0) { [self.mapView removeOverlays:self.routePolylines]; }
...
- (void)setNaviAnnotationVisibility:(BOOL)visible {
  if (visible == self.anntationVisible) { return; }
...
+ (MANaviRoute *)naviRouteForRailway:(AMapRailway *)railway {
  if (railway == nil || railway.uid.length == 0) { return nil; }
  ...
  for (int i = 0; i < stations.count - 1; i++) {
    ...
    coordinates[0] = CLLocationCoordinate2DMake(currentStation.location.latitude,
                                                currentStation.location.longitude);
    coordinates[1] =
    CLLocationCoordinate2DMake(nextStation.location.latitude, nextStation.location.longitude);
    ...
    annotation.coordinate = CLLocationCoordinate2DMake(currentStation.location.latitude,
                                                       currentStation.location.longitude);
...
+ (MANaviRoute *)naviRouteForTaxi:(AMapTaxi *)taxi {
  ...
  coordinates[1] =
  CLLocationCoordinate2DMake(taxi.destination.latitude, taxi.destination.longitude);
    ...
    if (stepPolyline != nil) {
      ...
      if (idx > 0) {
        [self replenishPolylinesForWalkingWith:stepPolyline
                                  LastPolyline:[self polylineForStep:[walking.steps
                                                                      objectAtIndex:idx - 1]]
                                     Polylines:polylines
                                       Walking:walking];
      }
...
+ (MANaviRoute *)naviRouteForSegment:(AMapSegment *)segment index:(NSInteger)idx {
  ...
  [self replenishPolylinesForSegment:walkingRoute.routePolylines
                     busLinePolyline:busLinePolyline
                             Segment:segment
                           polylines:polylines];
...
+ (void)replenishPolylinesForStartPoint:(AMapGeoPoint *)start
  ...
  if (startDashPolyline) { [polylines addObject:startDashPolyline]; }
  if (endDashPolyline) { [polylines addObject:endDashPolyline]; }
}
...
+ (void)replenishPolylinesForWalkingWith:(MAPolyline *)stepPolyline
  ...
  if (endCoor.latitude != startCoor.latitude || endCoor.longitude != startCoor.longitude) {
    ...
    if (dashPolyline) { [polylines addObject:dashPolyline]; }
  }
}
...
+ (void)replenishPolylinesForSegment:(NSArray *)walkingPolylines
  ...
  if (walkingPolylines.count != 0) {
    ...
    if (busLinePolyline) {
      CLLocationCoordinate2D startCoor =
      CLLocationCoordinate2DMake(walkingEndPoint.latitude, walkingEndPoint.longitude);
...
+ (void)replenishPolylinesForTransit:(AMapSegment *)lastSegment
  ...
  if (lastSegment) {
    ...
    if (busLinePolyline != nil) {
      [busLinePolyline getCoordinates:&startCoor
                                range:NSMakeRange(busLinePolyline.pointCount - 1, 1)];
    } else if (lastSegment.railway.arrivalStation) {
      startCoor = CLLocationCoordinate2DMake(lastSegment.railway.arrivalStation.location.latitude,
                                             lastSegment.railway.arrivalStation.location.longitude);
    } else {
    ...
    if ((segment).walking && [(segment).walking.steps count] != 0) {
      ...
      if (busLinePolyline != nil) {
        ...
        endCoor = CLLocationCoordinate2DMake(segment.railway.departureStation.location.latitude,
                                             segment.railway.departureStation.location.longitude);
      }
    }
...
+ (LineDashPolyline *)replenishPolylineWithStart:(CLLocationCoordinate2D)startCoor
                                             end:(CLLocationCoordinate2D)endCoor {
  if (!CLLocationCoordinate2DIsValid(startCoor) || !CLLocationCoordinate2DIsValid(endCoor)) {
    return nil;
  }
  
  double distance = MAMetersBetweenMapPoints(MAMapPointForCoordinate(startCoor),
                                             MAMapPointForCoordinate(endCoor));
...
+ (MAPolyline *)multiColoredPolylineWithFutureDrivePath:(AMapFutureTimeInfoElement *)element
                                         polylineColors:(NSArray **)polylineColors {
  if (!element.tmcs.count) { return nil; }
  ...
  for (int i = 0; i < element.tmcs.count; i++) {
    [coorArray
     addObjectsFromArray:[self coordinateArrayWithPolylineString:element.tmcs[i].polyline]];
    [mutablePolylineColors addObject:[self colorWithTrafficStatus:element.tmcs[i].status]];
    NSInteger coorCount = coorArray.count;
    if (i != element.tmcs.count - 1) { [drawColorIndex addObject:@(coorCount)]; }
  }
  
  if (!coorArray.count) { return nil; }
  
  CLLocationCoordinate2D *runningCoords =
  (CLLocationCoordinate2D *)malloc(coorArray.count * sizeof(CLLocationCoordinate2D));
  ...
  MAMultiPolyline *polyline = [MAMultiPolyline polylineWithCoordinates:runningCoords
                                                                 count:coorArray.count
                                                      drawStyleIndexes:drawColorIndex];
  
  if (polylineColors) { *polylineColors = [mutablePolylineColors copy]; }
  return polyline;
}

#pragma mark - colored route

+ (MAPolyline *)multiColoredPolylineWithDrivePath:(AMapPath *)path
                                   polylineColors:(NSArray **)polylineColors
  ...
  [path.steps
   enumerateObjectsUsingBlock:^(AMapStep *_Nonnull step, NSUInteger idx, BOOL *_Nonnull stop) {
  ...
  [mutablePolylineTextures
   addObject:[self imageWithTrafficStatus:tmcs.firstObject.status isSelected:NO]];
  [mutablePolylineTexturesSelect
   addObject:[self imageWithTrafficStatus:tmcs.firstObject.status isSelected:YES]];
  
  [indexes addObject:@(0)];
  [coordinates addObject:[coorArray objectAtIndex:0]];
  for (; i < coorArray.count; ++i) {
    double oneDis = [self calcDistanceBetweenCoor:[self coordinateWithString:coorArray[i - 1]]
                                          andCoor:[self coordinateWithString:coorArray[i]]];
    if (sumLength + oneDis >= curTrafficLength) {
      ...
      } else // 需要插入一个点
      {
        ...
        NSString *extrnPoint = [self calcPointWithStartPoint:[coorArray objectAtIndex:i - 1]
                                                    endPoint:[coorArray objectAtIndex:i]
      ...
      [mutablePolylineTextures
       addObject:[self imageWithTrafficStatus:tmcs[statusesIndex].status isSelected:NO]];
      [mutablePolylineTexturesSelect
       addObject:[self imageWithTrafficStatus:tmcs[statusesIndex].status isSelected:YES]];
    } else {
      [coordinates addObject:[coorArray objectAtIndex:i]];
      sumLength += oneDis;
    }
  ...
  CLLocationCoordinate2D *runningCoords =
  (CLLocationCoordinate2D *)malloc(count * sizeof(CLLocationCoordinate2D));
  ...
  CustomMAMultiPolyline *polyline = [CustomMAMultiPolyline polylineWithCoordinates:runningCoords
                                                                             count:count
                                                                  drawStyleIndexes:array2];
  ...
  if (polylineColors) { *polylineColors = [mutablePolylineColors copy]; }
  if (coordinates) { *coords = [coordinates copy]; }
  return polyline;
}
...
+ (UIImage *)imageWithTrafficStatus:(NSString *)status isSelected:(BOOL)isSelected {
  ...
  if (!isSelected) { imageName = [NSString stringWithFormat:@"%@_unselected", imageName]; }
...
+ (double)calcDistanceBetweenCoor:(CLLocationCoordinate2D)coor1
                          andCoor:(CLLocationCoordinate2D)coor2 {
...
+ (CLLocationCoordinate2D)coordinateWithString:(NSString *)string {
  ...
  if (coorArray.count != 2) { return kCLLocationCoordinate2DInvalid; }
...
+ (instancetype)naviRouteForFuturePath:(AMapFutureTimeInfoElement *)element
                          withNaviType:(MANaviAnnotationType)type
                           showTraffic:(BOOL)showTraffic
                            startPoint:(AMapGeoPoint *)start
                              endPoint:(AMapGeoPoint *)end {
...
- (instancetype)initWithTransit:(AMapTransit *)transit
  ...
  if (transit == nil || transit.segments.count == 0) { return self; }
  ...
  [transit.segments enumerateObjectsUsingBlock:^(AMapSegment *segment, NSUInteger idx, BOOL *stop) {
    ...
    if (idx > 0) {
      [MANaviRoute replenishPolylinesForTransit:[transit.segments objectAtIndex:idx - 1]
...
- (instancetype)initWithPath:(AMapPath *)path
                withNaviType:(MANaviAnnotationType)type
                   timeInfos:(AMapFutureTimeInfoElement *)element
                 showTraffic:(BOOL)showTraffic
                  startPoint:(AMapGeoPoint *)start
                    endPoint:(AMapGeoPoint *)end {
  self = [self init];
  ...
  if (showTraffic && type == MANaviAnnotationTypeFutureDrive) {
    ...
    MAPolyline *polyline = [MANaviRoute multiColoredPolylineWithFutureDrivePath:element
    ...
    if (polyline) {
      ...
      self.multiPolylineColors = polylineColors;
             ...
             (type == MANaviAnnotationTypeDrive || type == MANaviAnnotationTypeTruck)) {
    NSArray *polylineColors = nil;
    NSArray *coordinates = nil;
    
    MAPolyline *polyline = [MANaviRoute multiColoredPolylineWithDrivePath:path
                                                           polylineColors:&polylineColors
    ...
    [path.steps enumerateObjectsUsingBlock:^(AMapStep *step, NSUInteger idx, BOOL *stop) {
      ...
      if (stepPolyline != nil) {
        ...
        if (idx > 0) {
          // 填充step和step之间的空隙
          [MANaviRoute
           replenishPolylinesForPathWith:stepPolyline
           lastPolyline:[MANaviRoute
                         polylineForStep:[path.steps objectAtIndex:idx - 1]]
           Polylines:polylines];
        }
      }
    }];
