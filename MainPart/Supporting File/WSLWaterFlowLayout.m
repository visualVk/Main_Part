...
//    CGRect frame = CGRectMake(0, MAX(0, MIN(self.collectionViewContentSize.height - DEVICE_HEIGHT + NavigationContentTop + TabBarHeight, contentOffset.y)), headerSize.width, headerSize.height);
      CGRect frame = CGRectMake(0, MAX(0, contentOffset.y), headerSize.width, headerSize.height);
...
- (BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds{
  QMUILogInfo(@"water flow",@"x=%f,y=%f,width=%f,height=%f",newBounds.origin.x,newBounds.origin.y,newBounds.size.width,newBounds.size.height);
  return YES;
}
//返回内容高度
