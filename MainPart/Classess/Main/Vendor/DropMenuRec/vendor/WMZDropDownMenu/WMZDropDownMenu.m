...
@interface WMZDropDownMenu () <UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate,
...
- (void)resetConfig {
  if (self.titleView) {
    for (UIView *view in self.titleView.subviews) { [view removeFromSuperview]; }
    [self.titleView removeFromSuperview];
  }
  for (UIView *view in self.subviews) { [view removeFromSuperview]; }
  if (self.titleBtnArr) { [self.titleBtnArr removeAllObjects]; }
...
- (void)updateUI {
  ...
  if ([[WMZDropMenuTool getCurrentVC] respondsToSelector:@selector(viewWillDisappear:)]) {
    // hook监听当前控制器消失
    MenuWeakSelf(
                 self)[[WMZDropMenuTool getCurrentVC] aspect_hookSelector:@selector(viewWillDisappear:)
  ...
  //相互排斥的标题
  if (self.delegate &&
      [self.delegate respondsToSelector:@selector(mutuallyExclusiveSectionsWithMenu:)]) {
  ...
  for (int i = 0; i < self.titleArr.count; i++) {
    ...
    btn.frame = CGRectMake(tmp ? CGRectGetMaxX(tmp.frame) : 0, 0,
                           self.titleView.frame.size.width /
                           (self.param.wMenuTitleEqualCount < self.titleArr.count
                            ? self.titleArr.count
                            : self.param.wMenuTitleEqualCount),
                           self.titleView.frame.size.height);
    //外部自定义标题按钮
    NSDictionary *config = nil;
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(menu:customTitleInSection:withTitleBtn:)]) {
    ...
    btn.frame = CGRectMake(tmp ? (CGRectGetMaxX(tmp.frame) + offset) : offset, y,
                           btn.frame.size.width - offset, btn.frame.size.height - y * 2);
  ...
  forControlEvents:UIControlEventTouchUpInside];
    ...
    if (i == self.titleArr.count - 1) {
      if (dictionary && dic[@"lastFix"]) {
        btn.frame = CGRectMake(CGRectGetWidth(self.frame) - self.param.wFixBtnWidth, 0,
                               self.param.wFixBtnWidth, self.titleView.frame.size.height);
        if ([[self subviews] indexOfObject:btn] == NSNotFound) { [self addSubview:btn]; }
        ...
        btn.frame = CGRectMake(CGRectGetWidth(self.frame) - self.param.wFixBtnWidth, y,
                               self.param.wFixBtnWidth, tmp.frame.size.height);
    ...
    if ([self.titleBtnArr indexOfObject:btn] == NSNotFound) { [self.titleBtnArr addObject:btn]; }
  ...
  //显示边框
  if (self.param.wBorderShow) { [self addBoder]; }
}
#pragma - mark 添加边框
- (void)addBoder {
  [self.titleBtnArr
   enumerateObjectsUsingBlock:^(WMZDropMenuBtn *btn, NSUInteger idx, BOOL *_Nonnull stop) {
    if (idx != self.titleBtnArr.count - 1) {
      [WMZDropMenuTool viewPathWithColor:MenuColor(0x999999)
                                PathType:MenuShadowPathRight
                               PathWidth:1
                             heightScale:0.5
                                  button:btn];
    }
  ...
  [WMZDropMenuTool viewPathWithColor:MenuColor(0x999999)
                            PathType:MenuShadowPathTop
                           PathWidth:1
                         heightScale:1
                              button:self];
  [WMZDropMenuTool viewPathWithColor:MenuColor(0x999999)
                            PathType:MenuShadowPathBottom
                           PathWidth:1
                         heightScale:1
                              button:self];
}
...
- (UITableViewCell *)tableView:(WMZDropTableView *)tableView
         cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  NSArray *data = [self.dataDic objectForKey:tableView.dropIndex.key];
  WMZDropTree *tree = data[indexPath.row];
  //自定义
  if (self.delegate && [self.delegate respondsToSelector:@selector
                        (menu:cellForUITableView:AtIndexPath:dataForIndexPath:)]) {
    UITableViewCell *cell = [self.delegate menu:self
                             cellForUITableView:tableView
                                    AtIndexPath:indexPath
                               dataForIndexPath:tree];
    if (cell && [cell isKindOfClass:[UITableViewCell class]]) { return cell; }
  }
  WMZDropTableViewCell *cell =
  [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([WMZDropTableViewCell class])];
  if (!cell) {
    cell = [[WMZDropTableViewCell alloc]
            initWithStyle:UITableViewCellStyleDefault
            reuseIdentifier:NSStringFromClass([WMZDropTableViewCell class])];
  }
  if ([tree isKindOfClass:[WMZDropTree class]]) {
    ...
    tree.isSelected ? UIColor.systemBlueColor : self.param.wCollectionViewCellTitleColor;
...
- (UIView *)tableView:(WMZDropTableView *)tableView viewForFooterInSection:(NSInteger)section {
  if (self.delegate &&
      [self.delegate respondsToSelector:@selector(menu:footViewForUITableView:AtDropIndexPath:)]) {
    UITableViewHeaderFooterView *footView = [self.delegate menu:self
                                         footViewForUITableView:tableView
                                                AtDropIndexPath:tableView.dropIndex];
    if (footView && [footView isKindOfClass:[UITableViewHeaderFooterView class]]) {
      return footView;
    }
  }
  WMZDropTableViewFootView *footView = [tableView
                                        dequeueReusableHeaderFooterViewWithIdentifier:NSStringFromClass(
                                                                                                        [WMZDropTableViewFootView class])];
  if (self.delegate &&
      [self.delegate respondsToSelector:@selector(menu:titleForFootViewAtDropIndexPath:)]) {
...
- (UIView *)tableView:(WMZDropTableView *)tableView viewForHeaderInSection:(NSInteger)section {
  if (self.delegate &&
      [self.delegate respondsToSelector:@selector(menu:headViewForUITableView:AtDropIndexPath:)]) {
    UITableViewHeaderFooterView *headView = [self.delegate menu:self
                                         headViewForUITableView:tableView
                                                AtDropIndexPath:tableView.dropIndex];
    if (headView && [headView isKindOfClass:[UITableViewHeaderFooterView class]]) {
      return headView;
    }
  }
  WMZDropTableViewHeadView *headView = [tableView
                                        dequeueReusableHeaderFooterViewWithIdentifier:NSStringFromClass(
                                                                                                        [WMZDropTableViewHeadView class])];
  
  if (self.delegate &&
      [self.delegate respondsToSelector:@selector(menu:titleForHeadViewAtDropIndexPath:)]) {
    NSString *title = [self.delegate menu:self titleForHeadViewAtDropIndexPath:tableView.dropIndex];
    headView.textLa.text = title;
  }
  return headView;
}
- (CGFloat)tableView:(WMZDropTableView *)tableView
...
- (UICollectionViewCell *)collectionView:(WMZDropCollectionView *)collectionView
  ...
  if (self.delegate &&
      [self.delegate respondsToSelector:@selector(menu:cellForUICollectionView:AtDropIndexPath
                                                  :AtIndexPath:dataForIndexPath:)]) {
    UICollectionViewCell *cell = [self.delegate menu:self
                             cellForUICollectionView:collectionView
                                     AtDropIndexPath:path
                                         AtIndexPath:indexPath
                                    dataForIndexPath:tree];
    if (cell && [cell isKindOfClass:[UICollectionViewCell class]]) { return cell; }
  }
  if (path.UIStyle == MenuUICollectionRangeTextField) {
    //默认视图
    WMZMenuTextFieldCell *cell = (WMZMenuTextFieldCell *)[collectionView
                                                          dequeueReusableCellWithReuseIdentifier:NSStringFromClass([WMZMenuTextFieldCell class])
    ...
    if ([tree isKindOfClass:[WMZDropTree class]]) {
      ...
      tree.lowPlaceholder = cell.lowText.placeholder = tree.lowPlaceholder;
      ...
      MenuWeakSelf(cell) cell.myBlock =
      ^(UITextField *_Nonnull textField, NSString *_Nonnull string) {
        MenuStrongSelf(weakObject) if (textField == cell.lowText) {
          strongObject.lowT = string;
          if (strongObject.lowT) { tree.rangeArr[0] = strongObject.lowT; }
        }
        else {
          strongObject.highT = string;
          if (strongObject.highT) { tree.rangeArr[1] = strongObject.highT; }
        }
        tree.isSelected = YES;
      };
    ...
    //默认视图
    WMZMenuCell *cell = (WMZMenuCell *)[collectionView
                                        dequeueReusableCellWithReuseIdentifier:NSStringFromClass([WMZMenuCell class])
                                        forIndexPath:indexPath];
    if ([tree isKindOfClass:[WMZDropTree class]]) {
      cell.btn.backgroundColor = tree.isSelected ? self.param.wCollectionViewCellSelectBgColor
      : self.param.wCollectionViewCellBgColor;
      [cell.btn setTitleColor:tree.isSelected ? self.param.wCollectionViewCellSelectTitleColor
                             : self.param.wCollectionViewCellTitleColor
                     forState:UIControlStateNormal];
      [cell.btn setTitle:tree.name forState:UIControlStateNormal];
      [cell.btn setImage:tree.image ? [UIImage bundleImage:tree.image] : nil
                forState:UIControlStateNormal];
      cell.btn.layer.borderColor = tree.isSelected
      ? self.param.wCollectionViewCellSelectTitleColor.CGColor
      : self.param.wCollectionViewCellTitleColor.CGColor;
      cell.btn.layer.borderWidth = tree.isSelected ? self.param.wCollectionViewCellBorderWith : 0;
    }
    return cell;
  }
}
- (NSInteger)collectionView:(WMZDropCollectionView *)collectionView
...
- (CGSize)collectionView:(WMZDropCollectionView *)collectionView
                  layout:(UICollectionViewLayout *)collectionViewLayout
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
  WMZDropIndexPath *path = collectionView.dropArr[indexPath.section];
  ;
  return CGSizeMake(
                    (floor((collectionView.frame.size.width -
                            self.param.wCollectionViewCellSpace * (path.collectionCellRowCount + 1)) /
                           (CGFloat)path.collectionCellRowCount)),
                    path.cellHeight);
}
- (UICollectionReusableView *)collectionView:(WMZDropCollectionView *)collectionView
           viewForSupplementaryElementOfKind:(NSString *)kind
                                 atIndexPath:(NSIndexPath *)indexPath {
  if (kind == UICollectionElementKindSectionHeader) {
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector
         (menu:headViewForUICollectionView:AtDropIndexPath:AtIndexPath:)]) {
      UICollectionReusableView *headView =
      [self.delegate menu:self
headViewForUICollectionView:collectionView
          AtDropIndexPath:collectionView.dropArr[indexPath.section]
              AtIndexPath:indexPath];
    ...
    WMZDropIndexPath *path = collectionView.dropArr[indexPath.section];
    WMZDropCollectionViewHeadView *headerView = [collectionView
                                                 dequeueReusableSupplementaryViewOfKind:kind
                                                 withReuseIdentifier:NSStringFromClass(
                                                                                       [WMZDropCollectionViewHeadView class])
                                                 forIndexPath:indexPath];
    headerView.accessTypeBtn.hidden = !path.showExpand;
    headerView.accessTypeBtn.selected = !path.expand;
    [headerView.accessTypeBtn setImage:[UIImage bundleImage:@"menu_xiangshang"]
                              forState:UIControlStateNormal];
    [headerView.accessTypeBtn setImage:[UIImage bundleImage:@"menu_xiangxia"]
                              forState:UIControlStateSelected];
    headerView.dropIndexPath = path;
    [headerView.accessTypeBtn addTarget:self
                                 action:@selector(expandAction:)
                       forControlEvents:UIControlEventTouchUpInside];
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(menu:titleForHeadViewAtDropIndexPath:)]) {
      NSString *title = [self.delegate menu:self
            titleForHeadViewAtDropIndexPath:collectionView.dropArr[indexPath.section]];
    ...
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector
         (menu:footViewForUICollectionView:AtDropIndexPath:AtIndexPath:)]) {
      UICollectionReusableView *footView =
      [self.delegate menu:self
footViewForUICollectionView:collectionView
          AtDropIndexPath:collectionView.dropArr[indexPath.section]
              AtIndexPath:indexPath];
    ...
    WMZDropCollectionViewFootView *footerView = [collectionView
                                                 dequeueReusableSupplementaryViewOfKind:kind
                                                 withReuseIdentifier:NSStringFromClass(
                                                                                       [WMZDropCollectionViewFootView class])
                                                 forIndexPath:indexPath];
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(menu:titleForFootViewAtDropIndexPath:)]) {
      NSString *title = [self.delegate menu:self
            titleForFootViewAtDropIndexPath:collectionView.dropArr[indexPath.section]];
...
- (void)collectionView:(WMZDropCollectionView *)collectionView
...
- (void)titleAction:(WMZDropMenuBtn *)sender {
  ...
  if (self.selectTitleBtn && self.selectTitleBtn.tag != sender.tag &&
    ...
    if (currentDrop.editStyle != MenuEditReSetCheck && currentDrop.editStyle != MenuEditCheck &&
  ...
  if (currentDrop.editStyle == MenuEditReSetCheck) {
    if (!self.close) { [self closeView]; }
    [self.selectTitleBtn setTitleColor:self.param.wCollectionViewCellSelectTitleColor
                              forState:UIControlStateSelected];
    sender.selected = YES;
    if (!sender.click) {
      [sender setImage:[UIImage bundleImage:sender.selectImage] forState:UIControlStateSelected];
      [sender setImage:[UIImage bundleImage:sender.selectImage]
              forState:UIControlStateSelected | UIControlStateHighlighted];
    } else {
      [sender setImage:[UIImage bundleImage:sender.reSelectImage] forState:UIControlStateSelected];
      [sender setImage:[UIImage bundleImage:sender.reSelectImage]
              forState:UIControlStateSelected | UIControlStateHighlighted];
    }
  ...
  if ([self.selectTitleBtn isSelected] && self.param.wMenuLine) {
    ...
    if (self.param.wJDCustomLine) { self.param.wJDCustomLine(self.selectTitleBtn.line); }
  } else {
    [self.selectTitleBtn hidenLine];
  }
}
#pragma - mark 关闭方法
- (void)closeView {
  if (self.keyBoardShow) { [[[UIApplication sharedApplication] keyWindow] endEditing:YES]; };
  //移除嵌套层
  BOOL exist = NO;
    ...
    for (WMZDropMenuBtn *btn in self.titleBtnArr) {
      if (btn != self.selectTitleBtn) {
        if ([btn isSelected] && self.param.wMenuLine) {
          [btn showLine:@{}];
          if (self.param.wJDCustomLine) { self.param.wJDCustomLine(btn.line); }
        }
      }
    }
  } else {
...
- (void)dataChangeAction:(NSInteger)section {
  ...
  for (WMZDropIndexPath *drop in self.dropPathArr) {
    if (drop.section == section) {
      ...
      [arr enumerateObjectsUsingBlock:^(WMZDropTree *_Nonnull obj, NSUInteger idx,
                                        BOOL *_Nonnull stop) {
...
- (void)closeAction {
  WMZDropIndexPath *currentDrop = [self getTitleFirstDropWthTitleBtn:self.selectTitleBtn];
  if (self.hook) { currentDrop.hideAnimalStyle = MenuHideAnimalNone; }
  //动画
  MenuWeakSelf(self)
  [self hideAnimal:currentDrop.hideAnimalStyle
              view:self.dataView
           durtion:menuAnimalTime
             block:^{ MenuStrongSelf(weakObject)[strongObject.dataView removeFromSuperview]; }];
  [self.shadowView removeFromSuperview];
  self.close = YES;
  ...
  if (self.delegate && [self.delegate respondsToSelector:@selector(menu:getAllSelectData:)]) {
    ...
    for (WMZDropIndexPath *drop in self.dropPathArr) {
      ...
      [arr enumerateObjectsUsingBlock:^(WMZDropTree *_Nonnull obj, NSUInteger idx,
                                        BOOL *_Nonnull stop) {
        if ([obj isKindOfClass:[WMZDropTree class]]) {
          if (obj.isSelected) { [allSelectArr addObject:obj]; }
        }
      }];
    }
...
- (void)openView {
  ...
  for (UIView *view in [MenuWindow subviews]) {
    ...
    if ((view.tag == 10086 && view != self.shadowView) ||
        (view.tag == 10087 && view != self.dataView)) {
      [[NSNotificationCenter defaultCenter] postNotificationName:notificationRemove
                                                          object:nil
                                                        userInfo:nil];
      break;
    }
  ...
  WMZDropIndexPath *currentDrop = [self getTitleFirstDropWthTitleBtn:self.selectTitleBtn];
  //阴影层frame改变
  self.shadowView.frame =
  [[self.shadomViewFrameDic objectForKey:@(currentDrop.showAnimalStyle)] CGRectValue];
  //数据层视图frame改变
  self.dataView.frame =
  [[self.dataViewFrameDic objectForKey:@(currentDrop.showAnimalStyle)] CGRectValue];
...
- (void)changeTitleArr:(BOOL)clear update:(BOOL)update {
  [self.titleBtnArr enumerateObjectsUsingBlock:^(WMZDropMenuBtn *_Nonnull obj, NSUInteger idx,
                                                 BOOL *_Nonnull stop) {
    if (obj.tag != self.selectTitleBtn.tag) {
      if ([self getTitleFirstDropWthTitleBtn:obj].connect ||
        ...
        obj.click = NO;
        obj.selectType = 0;
        if (update) { [self dealDataWithDelete:MenuDataDelete btn:obj]; }
        if (clear) {
...
#pragma - mark 点击选中方法
- (void)cellTap:(WMZDropIndexPath *)dropPath
           data:(NSArray *)arr
  ...
  if (dropPath.editStyle == MenuEditOneCheck) {
    ...
    for (WMZDropTree *tmpTree in arr) {
      if (tmpTree != tree) { tmpTree.isSelected = NO; }
    }
  ...
  //点击代理
  if (self.delegate && [self.delegate respondsToSelector:@selector
                        (menu:didSelectRowAtDropIndexPath:dataIndexPath:data:)]) {
    [self.delegate menu:self
didSelectRowAtDropIndexPath:dropPath
  ...
  for (UIView *view in self.showView) {
    if ([view isKindOfClass:[WMZDropTableView class]]) {
      ...
      if ([updateDrop indexOfObject:ta.dropIndex] != NSNotFound) {
        [UIView performWithoutAnimation:^{ [ta reloadData]; }];
      }
...
- (void)addTableView {
  //移除之前的view
  for (UIView *view in [self.dataView subviews]) { [view removeFromSuperview]; }
  //移除CALayer
  NSArray<CALayer *> *subLayers = self.dataView.layer.sublayers;
  NSArray<CALayer *> *removedLayers = [subLayers
  ...
  [removedLayers enumerateObjectsUsingBlock:^(CALayer *_Nonnull obj, NSUInteger idx,
                                              BOOL *_Nonnull stop) { [obj removeFromSuperlayer]; }];
...
- (void)addDropPathUI:(NSArray *)dropArr type:(MenuUIStyle)type {
  ...
  if (type == MenuUINone) { // none
    ...
    if (height == 0) {
      ...
      for (WMZDropIndexPath *path in dropArr) {
        if (path.showAnimalStyle == MenuShowAnimalLeft ||
          ...
          for (WMZDropTree *tree in dataArr) {
            if ([tree isKindOfClass:[WMZDropTree class]]) { sonHeight += tree.cellHeight; }
          }
      ...
      if (!screnFrame) {
        ...
        if (height >
            (Menu_Height * (self.param.wMaxHeightScale > 1 ? 1 : self.param.wMaxHeightScale))) {
          height =
          (Menu_Height * (self.param.wMaxHeightScale > 1 ? 1 : self.param.wMaxHeightScale));
        }
      }
    ...
    for (int i = 0; i < dropArr.count; i++) {
      ...
      tableView.frame = CGRectMake(tmpTa ? CGRectGetMaxX(tmpTa.frame) : 0, y,
                                   self.dataView.frame.size.width / dropArr.count, height - y);
    ...
    layout.sectionInset = UIEdgeInsetsMake(0, self.param.wCollectionViewCellSpace, 0,
                                           self.param.wCollectionViewCellSpace);
    ...
    if (self.param.wReginerCollectionHeadViews) {
      for (NSString *name in self.param.wReginerCollectionHeadViews) {
        [self.collectionView registerClass:NSClassFromString(name)
  ...
  CGRect rect = [[self.dataViewFrameDic
                  objectForKey:@([self getTitleFirstDropWthTitleBtn:self.selectTitleBtn].showAnimalStyle)]
                 CGRectValue];
  ...
  if (screnFrame) {
    if (self.param.wMainRadius) {
      [WMZDropMenuTool setView:self.dataView
                         Radii:CGSizeMake(self.param.wMainRadius, self.param.wMainRadius)
               RoundingCorners:screnFrame == MenuShowAnimalRight
    ...
    if (self.param.wMainRadius) {
      [WMZDropMenuTool setView:self.dataView
                         Radii:CGSizeMake(self.param.wMainRadius, self.param.wMainRadius)
               RoundingCorners:UIRectCornerBottomRight | UIRectCornerBottomLeft];
    }
  }
}
...
- (void)addHeadFootView:(NSArray *)connectViews screnFrame:(MenuShowAnimalStyle)screnFrame {
  //头部
  if (self.delegate &&
      [self.delegate respondsToSelector:@selector(menu:userInteractionHeadViewInSection:)]) {
    UIView *headView =
    [self.delegate menu:self userInteractionHeadViewInSection:self.selectTitleBtn.tag - 1000];
    if (headView) {
    ...
    if (screnFrame == MenuShowAnimalBoss) {
      ...
      [bossHeadView.leftBtn addTarget:self
  ...
  //尾部
  if (self.delegate &&
      [self.delegate respondsToSelector:@selector(menu:userInteractionFootViewInSection:)]) {
    UIView *footView =
    [self.delegate menu:self userInteractionFootViewInSection:self.selectTitleBtn.tag - 1000];
    if (footView) {
      for (UIView *connectView in connectViews) {
        CGRect rect = connectView.frame;
        ...
        if (screnFrame) {
          rect.size.height -=
          ((footViewFram.size.height +
            (MenuisIphoneX ? (self.param.wCollectionViewDefaultFootViewMarginY >= 20 ? 0 : 20)
             : 0)) +
           self.param.wCollectionViewDefaultFootViewMarginY +
           self.param.wCollectionViewDefaultFootViewPaddingY);
        }
        connectView.frame = rect;
        footViewFram.origin.y =
        CGRectGetMaxY(connectView.frame) + self.param.wCollectionViewDefaultFootViewPaddingY;
    ...
    for (UIView *connectView in connectViews) {
      if ([connectView isKindOfClass:[UICollectionView class]]) {
        ...
        if (ta.dropIndex.tapClose) { insert = NO; }
      }
    }
    if (!insert) return;
    WMZDropConfirmView *footView = [WMZDropConfirmView new];
    [footView.confirmBtn addTarget:self
                            action:@selector(confirmAction)
                  forControlEvents:UIControlEventTouchUpInside];
    [footView.resetBtn addTarget:self
    ...
    for (UIView *connectView in connectViews) {
      ...
      if (screnFrame) {
        CGRect rect = connectView.frame;
        rect.size.height -=
        (self.param.wDefaultConfirmHeight +
         (MenuisIphoneX ? (self.param.wCollectionViewDefaultFootViewMarginY >= 20
                           ? 0
                           : (20 - self.param.wCollectionViewDefaultFootViewMarginY))
          : 0) +
         self.param.wCollectionViewDefaultFootViewMarginY +
         self.param.wCollectionViewDefaultFootViewPaddingY);
        connectView.frame = rect;
      }
      footView.frame = CGRectMake(
                                  0, CGRectGetMaxY(connectView.frame) + self.param.wCollectionViewDefaultFootViewPaddingY,
                                  self.dataView.frame.size.width, self.param.wDefaultConfirmHeight);
      if (self.delegate &&
          [self.delegate respondsToSelector:@selector(menu:customDefauultCollectionFootView:)]) {
        [self.delegate menu:self customDefauultCollectionFootView:footView];
        [footView layoutSubviews];
        footView.frame = CGRectMake(footView.frame.origin.x,
                                    CGRectGetMaxY(connectView.frame) +
                                    self.param.wCollectionViewDefaultFootViewPaddingY,
                                    footView.frame.size.width, self.param.wDefaultConfirmHeight);
      }
    }
    self.confirmView = footView;
    [self.dataView addSubview:self.confirmView];
...
- (void)confirmAction {
    ...
    if ([tree.rangeArr[0] length] && [tree.rangeArr[1] length]) {
      if ([tree.rangeArr[1] floatValue] < [tree.rangeArr[0] floatValue]) {
        UIAlertController *alert = [UIAlertController
                                    ...
                                    message:[NSString stringWithFormat:@"%@不能大于%@",
                                             tree.lowPlaceholder,
                                             tree.highPlaceholder]
  ...
  if (self.delegate &&
      [self.delegate respondsToSelector:@selector
       (menu:didConfirmAtSection:selectNoramelData:selectStringData:)]) {
    NSMutableArray *nameArr = [NSMutableArray new];
    for (WMZDropTree *tree in self.selectArr) {
      if (!tree.name) {
        if (tree.rangeArr) { [nameArr addObject:tree.rangeArr]; }
      } else {
        [nameArr addObject:tree.name];
      }
    }
    [self.delegate menu:self
    didConfirmAtSection:self.selectTitleBtn.tag - 1000
      selectNoramelData:self.selectArr
       selectStringData:nameArr];
  }
  
  if (self.selectArr.count) {
    if ([self getTitleFirstDropWthTitleBtn:self.selectTitleBtn].showAnimalStyle ==
        MenuShowAnimalLeft ||
        [self getTitleFirstDropWthTitleBtn:self.selectTitleBtn].showAnimalStyle ==
        MenuShowAnimalRight) {
      [self changeTitleConfig:@{
        @"name" : self.selectTitleBtn.titleLabel.text
      }
                      withBtn:self.selectTitleBtn];
    } else {
      WMZDropTree *tree = self.selectArr[0];
      [self changeTitleConfig:@{
        @"name" : self.selectArr.count > 1 ? self.selectTitleBtn.titleLabel.text
...
- (WMZDropIndexPath *)getTitleFirstDropWthTitleBtn:(WMZDropMenuBtn *)btn {
  for (WMZDropIndexPath *path in self.dropPathArr) {
    if (path.section == btn.tag - 1000) { return path; }
  }
  return nil;
}
 ...
 */
- (BOOL)updateData:(NSArray *)arr
AtDropIndexPathSection:(NSInteger)section
AtDropIndexPathRow:(NSInteger)row {
  ...
  //更新标题
  [self updateTitle:currentDrop
          changeArr:[self.dataDic objectForKey:currentDrop.key]
         changeTree:nil];
  return result;
}
 ...
 */
- (BOOL)updateDataConfig:(NSDictionary *)changeData
  AtDropIndexPathSection:(NSInteger)section
      AtDropIndexPathRow:(NSInteger)row
          AtIndexPathRow:(NSInteger)indexPathRow {
  BOOL result = YES;
  WMZDropIndexPath *currentDrop = nil;
  for (WMZDropIndexPath *tmpDrop in self.dropPathArr) {
    if (tmpDrop.section == section && tmpDrop.row == row) {
  ...
  [dataArr
   enumerateObjectsUsingBlock:^(WMZDropTree *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
    if (idx == indexPathRow) {
...
//自动增加数组的配置
- (void)updateWithData:(NSArray *)arr
              dropPath:(WMZDropIndexPath *)currentDrop
        normalDropPath:(WMZDropIndexPath *)dropIndexPath
                  more:(BOOL)more {
  ...
  if (arr.count) {
    for (id dic in arr) {
      ...
      // cell高度
      if (self.delegate &&
          [self.delegate respondsToSelector:@selector(menu:heightAtDropIndexPath:AtIndexPath:)]) {
        CGFloat cellHeight =
        [self.delegate menu:self
      heightAtDropIndexPath:currentDrop
                AtIndexPath:[NSIndexPath indexPathForRow:[arr indexOfObject:dic]
                                               inSection:[self.dropPathArr
                                                          indexOfObject:currentDrop]]];
...
//更新标题
- (void)updateTitle:(WMZDropIndexPath *)currentDrop
          changeArr:(NSArray *)arr
         changeTree:(WMZDropTree *)tree {
  ...
  for (WMZDropIndexPath *dropPath in self.dropPathArr) {
    if (dropPath.section == currentDrop.section) {
      ...
      [data enumerateObjectsUsingBlock:^(WMZDropTree *_Nonnull obj, NSUInteger idx,
                                         BOOL *_Nonnull stop) {
        if (obj.isSelected) {
      ...
      if (dropPath.editStyle == MenuEditOneCheck) {
        ...
        if (sectionSelectArr.count > 1) {
          [sectionSelectArr enumerateObjectsUsingBlock:^(WMZDropTree *_Nonnull obj, NSUInteger idx,
                                                         BOOL *_Nonnull stop) {
  ...
  if (selectArr.count > 0) {
    ...
    if (currentDrop.UIStyle == MenuUITableView) {
      ...
      [selectArr enumerateObjectsUsingBlock:^(WMZDropTree *_Nonnull obj, NSUInteger idx,
                                              BOOL *_Nonnull stop) {
        if ([arr indexOfObject:obj] != NSNotFound) { [lastSelectArr addObject:obj]; }
      }];
    ...
    } else if (currentDrop.UIStyle == MenuUICollectionView ||
               currentDrop.UIStyle == MenuUICollectionRangeTextField) {
      if (selectArr.count == 1) {
        WMZDropTree *tree = selectArr[0];
        showTitle = tree.name;
      } else {
        //                showTitle = @"多选";
        showTitle = currentBtn.titleLabel.text;
      }
    }
