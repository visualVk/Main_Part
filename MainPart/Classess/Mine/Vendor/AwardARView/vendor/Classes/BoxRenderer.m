...
+ (void)getGLMatrix:(easyar_Matrix44F *)m result:(float (*) [16])a {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            (*a)[i + j * 4] = [[m.data objectAtIndex:(i * 4 + j)] floatValue];
        }
    }
}
...
- (void)render:(easyar_Matrix44F *)projectionMatrix cameraview:(easyar_Matrix44F *)cameraview size:(easyar_Vec2F *)size
{
    ...
    [BoxRenderer getGLMatrix:cameraview result:&cameraview_data];
    [BoxRenderer getGLMatrix:projectionMatrix result:&projectionMatrix_data];
