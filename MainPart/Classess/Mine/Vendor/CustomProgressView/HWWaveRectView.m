...
//  MainPart
//
//  Created by blacksky on 2020/3/18.
...
#define KHWWaveTopColor                                                                            \
[UIColor colorWithRed:0 / 255.0 green:191 / 255.0 blue:255 / 255.0 alpha:1.0f] //前面波浪颜色
#define KHWWaveBottomColor                                                                         \
[UIColor colorWithRed:0 / 255.0 green:191 / 255.0 blue:255 / 255.0 alpha:0.4f] //后面波浪颜色
@interface HWWaveRectView ()
...
- (void)drawWaveColor:(UIColor *)color offsetx:(CGFloat)offsetx offsety:(CGFloat)offsety {
  ...
  for (float next_x = 0.f; next_x <= self.frame.size.width; next_x++) {
    //正弦函数，绘制波形
    CGFloat next_y = _wave_amplitude * sin(_wave_cycle * next_x + _wave_offsetx +
                                           offsetx / self.bounds.size.width * 2 * M_PI) +
    _wave_offsety + offsety;
    if (next_x == 0) {
