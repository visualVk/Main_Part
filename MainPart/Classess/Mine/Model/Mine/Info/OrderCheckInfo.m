...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary {
  self = [super init];
  if (dictionary[kOrderCheckInfoCheckInfo] != nil &&
      [dictionary[kOrderCheckInfoCheckInfo] isKindOfClass:[NSArray class]]) {
...
- (NSDictionary *)toDictionary {
  ...
  if (self.dinnerAddress != nil) { dictionary[kOrderCheckInfoDinnerAddress] = self.dinnerAddress; }
  if (self.edDate != nil) { dictionary[kOrderCheckInfoEdDate] = self.edDate; }
  if (self.hotelAddress != nil) { dictionary[kOrderCheckInfoHotelAddress] = self.hotelAddress; }
  dictionary[kOrderCheckInfoHotelId] = @(self.hotelId);
  if (self.hotelName != nil) { dictionary[kOrderCheckInfoHotelName] = self.hotelName; }
  if (self.orderDate != nil) { dictionary[kOrderCheckInfoOrderDate] = self.orderDate; }
  if (self.orderId != nil) { dictionary[kOrderCheckInfoOrderId] = self.orderId; }
  dictionary[kOrderCheckInfoOrderStatus] = @(self.orderStatus);
  if (self.roomCombo != nil) { dictionary[kOrderCheckInfoRoomCombo] = self.roomCombo; }
  if (self.roomPrice != nil) { dictionary[kOrderCheckInfoRoomPrice] = self.roomPrice; }
  dictionary[kOrderCheckInfoRoomTypeId] = @(self.roomTypeId);
  if (self.roomTypeName != nil) { dictionary[kOrderCheckInfoRoomTypeName] = self.roomTypeName; }
  if (self.stDate != nil) { dictionary[kOrderCheckInfoStDate] = self.stDate; }
  return dictionary;
}
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
  ...
  if (self.edDate != nil) { [aCoder encodeObject:self.edDate forKey:kOrderCheckInfoEdDate]; }
  if (self.hotelAddress != nil) {
    [aCoder encodeObject:self.hotelAddress forKey:kOrderCheckInfoHotelAddress];
  }
  [aCoder encodeObject:@(self.hotelId) forKey:kOrderCheckInfoHotelId];
  ...
  if (self.orderId != nil) { [aCoder encodeObject:self.orderId forKey:kOrderCheckInfoOrderId]; }
  [aCoder encodeObject:@(self.orderStatus) forKey:kOrderCheckInfoOrderStatus];
  ...
  [aCoder encodeObject:@(self.roomTypeId) forKey:kOrderCheckInfoRoomTypeId];
  if (self.roomTypeName != nil) {
    [aCoder encodeObject:self.roomTypeName forKey:kOrderCheckInfoRoomTypeName];
  }
  if (self.stDate != nil) { [aCoder encodeObject:self.stDate forKey:kOrderCheckInfoStDate]; }
}
...
+ (NSDictionary *)mj_objectClassInArray {
  return @{ @"checkInfo" : [CheckInfo class] };
}
@end
