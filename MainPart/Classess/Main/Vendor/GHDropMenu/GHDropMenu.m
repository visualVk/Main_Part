...
@interface GHDropMenu () <
UICollectionViewDelegate, UICollectionViewDataSource, UITableViewDataSource,
UITableViewDelegate, GHDropMenuFilterTagItemDelegate, GHDropMenuFilterInputItemDelegate,
GHDropMenuFilterSingleInputItemDelegate, GHDropMenuTitleItemDelegate,
GHDropMenuFilterSectionHeaderDelegate, GHDropMenuFilterTimeChoseItemDelegate>
...
+ (instancetype)creatDropFilterMenuWidthConfiguration:(GHDropMenuModel *)configuration
  ...
  GHDropMenu *dropMenu =
  [[GHDropMenu alloc] initWithFrame:CGRectMake(0, 0, kGHScreenWidth, kGHScreenHeight)];
...
+ (instancetype)creatDropMenuWithConfiguration:(GHDropMenuModel *)configuration
                         ...
                         dropMenuTagArrayBlock:(DropMenuTagArrayBlock)dropMenuTagArrayBlock {
  GHDropMenu *dropMenu = [[GHDropMenu alloc]
                          initWithFrame:CGRectMake(0, frame.origin.y, frame.size.width, frame.size.height)];
...
- (instancetype) new {
  @throw [NSException exceptionWithName:NSInternalInconsistencyException
                                 reason:@"请使用方法 creatDropMenuWithConfiguration: or "
          @"creatDropFilterMenuWidthConfiguration: 代替初始化"
                               userInfo:nil];
}
- (instancetype)init {
  @throw [NSException exceptionWithName:NSInternalInconsistencyException
...
- (instancetype)initWithFrame:(CGRect)frame {
  if (self == [super initWithFrame:frame]) { [self defaultConfiguration]; }
  return self;
}
...
- (void)dismiss {
  ...
  [UIView animateWithDuration:self.durationTime
    ...
    if (dropMenuTitleModel.dropMenuType == GHDropMenuTypeTitle /** 普通菜单 */) {
      ...
      self.titleCover.frame = CGRectMake(0, self.tableY, kGHScreenWidth, 0);
      self.sure.frame = CGRectMake(0, CGRectGetMaxY(self.filter.frame), self.filter.width * 0.5,
                                   kFilterButtonHeight);
      self.reset.frame = CGRectMake(self.filter.width * .5, CGRectGetMaxY(self.filter.frame),
                                    self.filter.width * 0.5, kFilterButtonHeight);
    }
  }
                   completion:^(BOOL finished) {
    if (self.dropMenuShowType == GHDropMenuShowTypeOnlyFilter) { [self.layer setOpacity:0.0]; }
    self.isShow = NO;
    [self.tableView reloadData];
    [self.collectionView reloadData];
...
- (void)show {
  ...
  self.sure.alpha = 0;
  self.reset.alpha = 0;
  if (self.dropMenuShowType == GHDropMenuShowTypeOnlyFilter) { [self.layer setOpacity:1]; }
  GHDropMenuModel *dropMenuTitleModel = [self.titles by_ObjectAtIndex:self.currentIndex];
  if (dropMenuTitleModel.dropMenuType == GHDropMenuTypeTitle /** 筛选菜单 */) {
    ...
    self.reset.frame =
    CGRectMake(0, CGRectGetMaxY(self.filter.frame), kGHScreenWidth * 0.5, kFilterButtonHeight);
    self.sure.frame = CGRectMake(kGHScreenWidth * .5, CGRectGetMaxY(self.filter.frame),
                                 kGHScreenWidth * 0.5, kFilterButtonHeight);
    self.sure.alpha = 1;
    self.reset.alpha = 1;
    ...
    self.filter.frame = CGRectMake(kGHScreenWidth * 0.1, 0, kGHScreenWidth * 0.9,
                                   kGHScreenHeight - kFilterButtonHeight - kGHSafeAreaBottomHeight);
    [self.filterCover addSubview:self.sure];
    [self.filterCover addSubview:self.reset];
    self.reset.frame = CGRectMake(self.filter.x, self.filter.frame.size.height,
                                  self.filter.width * 0.5, kFilterButtonHeight);
    self.sure.frame =
    CGRectMake(self.filter.frame.size.width * 0.5 + kGHScreenWidth * 0.1,
               CGRectGetMaxY(self.filter.frame), self.filter.width * 0.5, kFilterButtonHeight);
    self.sure.alpha = 1;
    self.reset.alpha = 1;
  }
  [UIView animateWithDuration:self.durationTime
                   animations:^{
    if (dropMenuTitleModel.dropMenuType == GHDropMenuTypeTitle /** 普通菜单 */) {
      self.tableView.frame = CGRectMake(0, self.tableY, self.frame.size.width,
                                        dropMenuTitleModel.dataArray.count * self.cellHeight);
      self.titleCover.frame =
      CGRectMake(0, self.tableY, kGHScreenWidth,
                 kGHScreenHeight - self.menuHeight - kGHSafeAreaTopHeight);
    ...
    } else if (dropMenuTitleModel.dropMenuType ==
               GHDropMenuTypeOptionCollection /** 筛选菜单 */) {
      self.filter.frame = CGRectMake(0, self.tableY, self.frame.size.width, 500);
      self.titleCover.frame =
      CGRectMake(0, self.tableY, kGHScreenWidth,
                 kGHScreenHeight - self.menuHeight - kGHSafeAreaTopHeight);
      
      self.reset.frame = CGRectMake(0, CGRectGetMaxY(self.filter.frame),
                                    self.filter.width * 0.5, kFilterButtonHeight);
      self.sure.frame = CGRectMake(self.filter.width * .5, CGRectGetMaxY(self.filter.frame),
                                   self.filter.width * 0.5, kFilterButtonHeight);
    }
  }
                   completion:^(BOOL finished) {
    [UIView animateWithDuration:0.1
      ...
      if (dropMenuTitleModel.dropMenuType == GHDropMenuTypeFilter /** 筛选菜单 */) {
        self.filterCover.backgroundColor =
        [UIColor colorWithRed:0 green:0 blue:0 alpha:102.0 / 255];
      } else if (dropMenuTitleModel.dropMenuType == GHDropMenuTypeTitle) {
        self.titleCover.backgroundColor =
        [UIColor colorWithRed:0 green:0 blue:0 alpha:102.0 / 255];
      } else if (dropMenuTitleModel.dropMenuType == GHDropMenuTypeOptionCollection) {
        self.titleCover.backgroundColor =
        [UIColor colorWithRed:0 green:0 blue:0 alpha:102.0 / 255];
                     ...
                     completion:^(BOOL finished) { self.isShow = YES; }];
  }];
}
- (void)layoutSubviews {
...
- (void)resetMenuStatus {
  for (GHDropMenuModel *dropMenuModel in self.titles) { dropMenuModel.titleSeleted = NO; }
  [self.filter reloadData];
  [self.collectionView reloadData];
  [self dismiss];
}
...
- (void)dropMenuFilterSingleInputItem:(GHDropMenuFilterSingleInputItem *)item
  ...
  GHDropMenuModel *dropMenuTitleModel = [self.titles by_ObjectAtIndex:self.currentIndex];
  GHDropMenuModel *dropMenuSectionModel =
  [dropMenuTitleModel.sections by_ObjectAtIndex:dropMenuModel.indexPath.section];
  GHDropMenuModel *dropMenuTagModel =
  [dropMenuSectionModel.dataArray by_ObjectAtIndex:dropMenuModel.indexPath.row];
  dropMenuTagModel.singleInput = dropMenuModel.singleInput;
}

- (void)dropMenuFilterEndInputItem:(GHDropMenuFilterInputItem *)item
                     dropMenuModel:(GHDropMenuModel *)dropMenuModel {
...
- (void)dropMenuFilterTimeChoseItem:(GHDropMenuFilterTimeChoseItem *)item
                               type:(GHDropMenuFilterTimeChoseItemType)type
                      dropMenuModel:(GHDropMenuModel *)dropMenuModel {
  GHCustomAlertView *alert = [[GHCustomAlertView alloc] init];
  alert.alertHeight = 220;
  alert.alertTitle =
  type == GHDropMenuFilterTimeChoseItemTypeBeginTime ? @"选择开始时间" : @"选择结束时间";
  ...
  alert.dimissFinish = ^{ [self.filter reloadData]; };
  [alert show];
}
...
- (void)dropMenuFilterTagItem:(GHDropMenuFilterTagItem *)item
  ...
  /** 处理sectionDetails */
  [self actionSectionHeaderDetailsWithDropMenuModel:dropMenuModel
                               dropMenuSectionModel:dropMenuSectionModel];
  [self.filter reloadData];
}
...
- (void)actionSectionHeaderDetailsWithDropMenuModel:(GHDropMenuModel *)dropMenuModel
  ...
  for (GHDropMenuModel *dropMenuTagModel in dropMenuSectionModel.dataArray) {
    if (dropMenuTagModel.tagSeleted) { [details appendFormat:@"%@,", dropMenuTagModel.tagName]; }
  }
  if (details.length) { [details deleteCharactersInRange:NSMakeRange(details.length - 1, 1)]; }
...
- (void)actionMultipleWithDropMenuModel:(GHDropMenuModel *)dropMenuModel
  ...
  if (dropMenuSectionModel.isMultiple) {
    ...
    for (GHDropMenuModel *dropMenuTagModel in dropMenuSectionModel.dataArray) {
      if (dropMenuTagModel.tagSeleted) { currentSeletedStr = dropMenuTagModel.tagName; }
...
- (void)dropMenuTitleItem:(GHDropMenuTitleItem *)item
  ...
  if (dropMenuModel.titleSeleted) {
    ...
    for (GHDropMenuModel *model in self.titles) {
      if (model.identifier != dropMenuModel.identifier) { model.titleSeleted = NO; }
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (dropMenuTitleModel.dropMenuType == GHDropMenuTypeWaterFall) {
    GHDropMenuWaterFallCell *cell =
    (GHDropMenuWaterFallCell *)[tableView.dataSource tableView:self.tableView
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  dropMenuModel.indexPath =
  [NSIndexPath indexPathForRow:indexPath.row inSection:dropMenuTitleModel.indexPath.row];
  
  if (dropMenuTitleModel.dropMenuType == GHDropMenuTypeWaterFall) {
    GHDropMenuWaterFallCell *cell =
    [tableView dequeueReusableCellWithIdentifier:@"GHDropMenuWaterFallCellID"];
    cell.tags = dropMenuModel.waterFallTags;
    return cell;
    
  } else {
    NSString *cellIdentifier =
    [NSString stringWithFormat:@"GHDropMenuOptionCellID%ld%ld%ld", indexPath.section,
     indexPath.row, (long)dropMenuTitleModel.identifier];
    
    GHDropMenuOptionCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (cell == nil) {
      cell = [[GHDropMenuOptionCell alloc] initWithStyle:UITableViewCellStyleSubtitle
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (self.configuration.recordSeleted) { dropMenuModel.title = contentModel.title; }
  ...
  if (self.dropMenuTitleBlock) { self.dropMenuTitleBlock(contentModel); }
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView
  ...
  GHDropMenuModel *dropMenuTitleModel = [self.titles by_ObjectAtIndex:self.currentIndex];
  GHDropMenuModel *dropMenuSectionModel =
  [dropMenuTitleModel.sections by_ObjectAtIndex:indexPath.section];
  dropMenuSectionModel.indexPath = indexPath;
  if ([kind isEqualToString:UICollectionElementKindSectionHeader] &&
      self.filter == collectionView) {
    GHDropMenuFilterSectionHeader *header =
    [collectionView dequeueReusableSupplementaryViewOfKind:kind
                                       withReuseIdentifier:@"GHDropMenuFilterSectionHeaderID"
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  if (collectionView == self.collectionView) {
    ...
    GHDropMenuModel *dropMenuSectionModel =
    [dropMenuModel.sections by_ObjectAtIndex:indexPath.section];
    if (dropMenuSectionModel.filterCellType == GHDropMenuFilterCellTypeTag) {
      return CGSizeMake((dropMenuModel.menuWidth - (dropMenuModel.sectionCount + 1) * 10) /
                        dropMenuModel.sectionCount,
                        30.01f);
...
- (NSInteger)collectionView:(UICollectionView *)collectionView
  ...
  if (self.collectionView == collectionView) {
    ...
    if (dropMenuSectionModel.filterCellType == GHDropMenuFilterCellTypeTag) {
      return dropMenuSectionModel.sectionSeleted
      ? dropMenuSectionModel.dataArray.count
      : dropMenuSectionModel.dataArray.count > 3 ? 3
      : dropMenuSectionModel.dataArray.count;
    } else if (dropMenuSectionModel.filterCellType == GHDropMenuFilterCellTypeInput ||
               dropMenuSectionModel.filterCellType == GHDropMenuFilterCellTypeSingleInput) {
      return 1;
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  if (collectionView == self.collectionView) {
    NSString *identifier = [NSString stringWithFormat:@"GHDropMenuTitleItemID%ld%ld",
                            (long)indexPath.section, (long)indexPath.row];
    [collectionView registerClass:[GHDropMenuTitleItem class]
       forCellWithReuseIdentifier:identifier];
    GHDropMenuModel *dropMenuModel = [self.titles by_ObjectAtIndex:indexPath.row];
    dropMenuModel.indexPath = indexPath;
    GHDropMenuTitleItem *cell =
    [collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
    cell.dropMenuModel = dropMenuModel;
    cell.delegate = self;
    return cell;
  } else if (collectionView == self.filter) {
    NSString *identifier = [NSString stringWithFormat:@"GHDropMenuFilterTagItemID%ld%ld",
                            (long)indexPath.section, (long)indexPath.row];
    [self.filter registerClass:[GHDropMenuFilterTagItem class]
    forCellWithReuseIdentifier:identifier];
    ...
    [dropMenuTitleModel.sections by_ObjectAtIndex:indexPath.section];
    GHDropMenuModel *dropMenuTagModel =
    [dropMenuSectionModel.dataArray by_ObjectAtIndex:indexPath.row];
    dropMenuTagModel.indexPath = indexPath;
    if (dropMenuSectionModel.filterCellType == GHDropMenuFilterCellTypeTag) {
      GHDropMenuFilterTagItem *cell =
      [collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:indexPath];
      cell.dropMenuModel = dropMenuTagModel;
      cell.delegate = self;
      return cell;
    } else if (dropMenuSectionModel.filterCellType == GHDropMenuFilterCellTypeInput) {
      GHDropMenuFilterInputItem *cell =
      [collectionView dequeueReusableCellWithReuseIdentifier:@"GHDropMenuFilterInputItemID"
                                                forIndexPath:indexPath];
      cell.dropMenuModel = dropMenuTagModel;
      cell.delegate = self;
      return cell;
    } else if (dropMenuSectionModel.filterCellType == GHDropMenuFilterCellTypeSingleInput) {
      GHDropMenuFilterSingleInputItem *cell = [collectionView
                                               dequeueReusableCellWithReuseIdentifier:@"GHDropMenuFilterSingleInputItemID"
                                               forIndexPath:indexPath];
      cell.dropMenuModel = dropMenuTagModel;
      cell.delegate = self;
      return cell;
    } else if (dropMenuSectionModel.filterCellType == GHDropMenuFilterCellTypeTimeChose) {
      GHDropMenuFilterTimeChoseItem *cell =
      [collectionView dequeueReusableCellWithReuseIdentifier:@"GHDropMenuFilterTimeChoseItemID"
                                                forIndexPath:indexPath];
      cell.dropMenuModel = dropMenuTagModel;
      cell.delegate = self;
      return cell;
    } else {
      return [collectionView dequeueReusableCellWithReuseIdentifier:@"UICollectionViewCellID"
                                                       forIndexPath:indexPath];
    }
  } else {
    return [collectionView dequeueReusableCellWithReuseIdentifier:@"UICollectionViewCellID"
...
- (void)clickButton:(UIButton *)button {
  ...
  if (button.tag == GHDropMenuButtonTypeSure) {
    ...
    for (GHDropMenuModel *dropMenuSectionModel in dropMenuTitleModel.sections) {
      for (GHDropMenuModel *dropMenuTagModel in dropMenuSectionModel.dataArray) {
        if (dropMenuTagModel.tagSeleted) { [dataArray addObject:dropMenuTagModel]; }
    ...
    if (self.dropMenuTagArrayBlock) { self.dropMenuTagArrayBlock(dataArray.copy); }
...
- (UIView *)bottomView {
  if (_bottomView == nil) {
    ...
    _bottomView.frame =
    CGRectMake(self.filter.frame.origin.x,
               self.filter.frame.size.height + self.filter.frame.origin.y + kFilterButtonHeight,
               self.filter.frame.size.width, kGHSafeAreaBottomHeight);
  }
  return _bottomView;
}
- (UIControl *)titleCover {
  if (_titleCover == nil) {
    ...
    _titleCover.frame =
    CGRectMake(0, self.frame.size.height + kGHSafeAreaTopHeight, kGHScreenWidth, 0);
    [_titleCover addTarget:self
...
- (UIButton *)reset {
  if (_reset == nil) {
    ...
    _reset.frame = CGRectMake(self.filter.frame.origin.x, self.filter.frame.size.height,
                              self.filter.frame.size.width * 0.5, kFilterButtonHeight);
...
- (UIButton *)sure {
  if (_sure == nil) {
    ...
    _sure.frame = CGRectMake(kGHScreenWidth - self.filter.frame.size.width * 0.5,
                             self.filter.frame.size.height, self.filter.frame.size.width * 0.5,
                             kFilterButtonHeight);
...
- (UITableView *)tableView {
  if (_tableView == nil) {
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.tableY, 0, 0)
    ...
    [_tableView registerClass:[GHDropMenuOptionCell class]
       forCellReuseIdentifier:@"GHDropMenuOptionCellID"];
    [_tableView registerClass:[GHDropMenuWaterFallCell class]
       forCellReuseIdentifier:@"GHDropMenuWaterFallCellID"];
  }
  return _tableView;
}
...
- (UICollectionView *)filter {
  if (_filter == nil) {
    _filter = [[UICollectionView alloc]
               initWithFrame:CGRectMake(kGHScreenWidth * 0.1, 0, kGHScreenWidth * 0.9,
                                        kGHScreenHeight - kFilterButtonHeight -
                                        kGHSafeAreaBottomHeight)
               collectionViewLayout:self.filterFlowLayout];
    ...
    [_filter registerClass:[GHDropMenuFilterTagItem class]
forCellWithReuseIdentifier:@"GHDropMenuFilterTagItemID"];
    [_filter registerClass:[UICollectionViewCell class]
forCellWithReuseIdentifier:@"UICollectionViewCellID"];
    [_filter registerClass:[GHDropMenuFilterSectionHeader class]
forSupplementaryViewOfKind:UICollectionElementKindSectionHeader
       withReuseIdentifier:@"GHDropMenuFilterSectionHeaderID"];
    [_filter registerClass:[GHDropMenuFilterInputItem class]
forCellWithReuseIdentifier:@"GHDropMenuFilterInputItemID"];
    [_filter registerClass:[UICollectionReusableView class]
forSupplementaryViewOfKind:UICollectionElementKindSectionFooter
       withReuseIdentifier:@"UICollectionReusableViewID"];
    [_filter registerClass:[GHDropMenuFilterSingleInputItem class]
forCellWithReuseIdentifier:@"GHDropMenuFilterSingleInputItemID"];
    [_filter registerClass:[GHDropMenuFilterTimeChoseItem class]
forCellWithReuseIdentifier:@"GHDropMenuFilterTimeChoseItemID"];
  }
  return _filter;
}
...
- (UICollectionView *)collectionView {
  if (_collectionView == nil) {
    _collectionView = [[UICollectionView alloc]
                       initWithFrame:CGRectMake(kGHScreenWidth, 0, kGHScreenWidth, self.menuHeight)
    ...
    [_collectionView registerClass:[UICollectionViewCell class]
        forCellWithReuseIdentifier:@"UICollectionViewCellID"];
    [_collectionView registerClass:[GHDropMenuTitleItem class]
        forCellWithReuseIdentifier:@"GHDropMenuTitleItemID"];
  }
